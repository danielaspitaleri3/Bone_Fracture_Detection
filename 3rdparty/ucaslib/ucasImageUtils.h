...
namespace ucas
{
	...
	std::string imdepthstr(int ocv_depth);
	...
	double imVecMemoryGB(const std::vector<cv::Mat> & images);
	size_t imVecSubmatricesCount(const std::vector<cv::Mat> & images);
	bool imVecContainsSubmatrices(const std::vector<cv::Mat> & images);
	void imVecStat(const std::vector <cv::Mat> & images, double & min, double & max, double & mean, double & stdev);
	int countNonZero(const cv::Mat & img);
	bool isZero(const cv::Mat & img, int x, int y);
	void imVecCheck(const std::vector <cv::Mat> & images, int ocv_pixel_type = -1);

	// return (x,y) offset of 'data' in parent image (if any), otherwise return (0,0)
	cv::Point imOffsetInParent(const cv::Mat & img);

	// return parent image if 'data' is a ROI, otherwise return 'data' itself
	cv::Mat imParent(const cv::Mat & img);
	...
	// rotates 'img' by step*90 degrees
...
namespace ucas
{
	...
	cv::Mat binarizeColor(const cv::Mat & img, int threshold, int c);
	/*-------------------------------------------------------------------------------------------------------------------------*/
}
...
namespace ucas
{
	...
	std::vector<cv::Mat>						// vector of 'width' x 'width' uint8 binary images with non-black pixels being the SE
...
namespace ucas
{
	...
	std::string color2text(ucas::Color color);
}
