...
void imVecStatGeneric(const std::vector <cv::Mat> & images, double & min, double & max, double & mean, double & stdev)
{
...
void ucas::imVecStat(const std::vector <cv::Mat> & images, double & min, double & max, double & mean, double & stdev)
{
	imVecCheck(images);
...
template <typename pixel_type>
int countNonZeroGeneric(const cv::Mat & img)
{
	int zeros = 0;
	int channels = img.channels();
...
int ucas::countNonZero(const cv::Mat & img)
{
...
template <typename pixel_type>
bool isZeroGeneric(const cv::Mat & img, int x, int y)
{
	int channels = img.channels();
	const pixel_type* cvdata = img.ptr<pixel_type>(y);
...
bool ucas::isZero(const cv::Mat & img, int x, int y)
{
	int ocv_pixel_type = img.depth();
...
void ucas::imVecCheck(const std::vector <cv::Mat> & images, int ocv_pixel_type)
{
	// check preconditions
	if(images.empty())
...
template <typename pixel_type>
cv::Mat binarizeColorGeneric(const cv::Mat & img, int threshold, int c)
{
...
cv::Mat ucas::binarizeColor(const cv::Mat & img, int threshold, int c)
{
	int ocv_pixel_type = img.depth();
...
void ucas::stripewrite(const std::string & path, const std::vector <cv::Mat> & images, bool normalize)
{
	// check preconditions
	imVecCheck(images);
	cv::Size size( images[0].cols, images[0].rows);
